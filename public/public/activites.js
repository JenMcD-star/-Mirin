        ...
        if (response.status === 200) {
            if (data.count === 0) {
                ...
                for (let i = 0; i < data.activities.length; i++) {
                    ...
                    let rowHTML = `<td>${data.activities[i].name}</td><td>${data.activities[i].liftType}</td><td>${data.activities[i].weight}</td><td>${data.activities[i].reps}</td>${editButton}${deleteButton}`;
    ...
    document.addEventListener("click", async (e) => {
        ...
        if (e.target === logoff) {
            ...
            try {
                ...
                const data = await response.json();
                if (response.status === 200) {
                    activityName.value = data.activityName.name;
                    reps.value = data.activity.reps;
                    weight.value = data.activity.weight
                    liftType.value = data.activity.status;
                    showing.style.display = "none";
                    showing = editActivity;
                    ...
                    message.textContent = "The activities entry was not found";
                    thisEvent = new Event("startDisplay");
                    document.dispatchEvent(thisEvent);
                }
            } catch (err) {
